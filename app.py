...
def setup_page() -> None:
    ...
    hide_streamlit_cloud_floating_badge()


def hide_streamlit_cloud_floating_badge() -> None:
    """Remove Streamlit Cloud floating viewer badge/avatar from the page DOM."""
    components.html(
        """
        <script>
        (function() {
            ...
            const textOf = (element) => {
                try {
                    return [
                        ...
                        element.outerHTML ? element.outerHTML.slice(0, 1200) : ""
                    ].join(" ").toLowerCase();
                } catch (error) {
                    return "";
                }
            ...
            const isBottomRightFloating = (element) => {
                try {
                    ...
                    const nearBottom = rect.bottom >= win.innerHeight - 180;
                    const nearRight = rect.right >= win.innerWidth - 280;
                    const compact = rect.width > 8 && rect.width <= 360 && rect.height > 8 && rect.height <= 180;
                    const visible = style.display !== "none" && style.visibility !== "hidden" && Number(style.opacity || "1") > 0;

                    return nearBottom && nearRight && compact && visible;
                } catch (error) {
                    return false;
                }
            };

            const looksLikeCloudBadge = (element) => {
                const text = textOf(element);
                if (
                    text.includes("streamlit") ||
                    text.includes("share.streamlit.io") ||
                    text.includes("streamlit.io/cloud") ||
                    text.includes("github.com/edgarmoraesufrn") ||
                    text.includes("labcim-manager-beta") ||
                    text.includes("avatars.githubusercontent.com/u/29822154") ||
                    text.includes("viewerbadge") ||
                    text.includes("viewer-badge") ||
                    text.includes("hostedbadge") ||
                    text.includes("cloudbadge")
                ) {
                    return true;
                }
            ...
            const hideClosestFloatingContainer = (element) => {
                let current = element;
                let candidate = null;

                for (let i = 0; i < 8 && current && current !== doc.body && current !== doc.documentElement; i++) {
                    if (isBottomRightFloating(current)) {
                        candidate = current;
                    }
                    current = current.parentElement;
                }

                forceHide(candidate || element);
            };
            ...
            const cleanup = () => {
                try {
                    ...
                    elements.forEach((element) => {
                        if (element.getAttribute && element.getAttribute(HIDE_ATTR) === "true") return;

                        if (looksLikeCloudBadge(element)) {
                            hideClosestFloatingContainer(element);
                            return;
                        }

                        if (isBottomRightFloating(element)) {
                            const text = textOf(element);
                            const hasImg = !!element.querySelector("img, svg");
            ...
            cleanup();
            [50, 150, 300, 700, 1200, 2000].forEach((delay) => setTimeout(cleanup, delay));

            try {
                if (!doc.__labcimCloudBadgeObserverInstalled) {
                    doc.__labcimCloudBadgeObserverInstalled = true;
