...
def init_db(conn: DatabaseConnection) -> None:
    ...
    _add_column(conn, "supplies", "inactive_reason", "TEXT")
    _add_column(conn, "supplies", "inactive_by_id", "INTEGER")
    _add_column(conn, "supplies", "inactive_at", "TEXT")
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bookings_equipment_period_status ON bookings (equipment_id, start_datetime, end_datetime, status)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bookings_start_datetime ON bookings (start_datetime)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_booking_status_history_booking_changed ON booking_status_history (booking_id, changed_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_booking_status_history_changed_at ON booking_status_history (changed_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_booking_status_history_changed_by ON booking_status_history (changed_by_id, changed_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_supply_movements_movement_date ON supply_movements (movement_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_supply_movements_project_service_date ON supply_movements (project_id, service_id, movement_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_supply_movements_lot_date ON supply_movements (supply_lot_id, movement_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_supply_lots_active_expiration ON supply_lots (is_active, expiration_date)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_attachments_entity_role_active ON attachments (entity_type, attachment_role, is_active, entity_id)")
    ...
    conn.execute("UPDATE supplies SET active = 1 WHERE active IS NULL")
...
BOOKING_STATUSES = {"scheduled", "done", "cancelled", "no_show"}
...
def _clean_booking_status(value: Any) -> str:
    return "" if value is None else str(value).strip()


def _select_booking_for_status_update(conn: DatabaseConnection, booking_id: int) -> Any:
...
def _record_booking_status_history(
    conn: DatabaseConnection,
    *,
    booking_id: int,
    ...
    conn.execute(
        ...
        INSERT INTO booking_status_history (
            booking_id, previous_status, new_status, changed_by_id,
...
def change_booking_status(
    ...
    new_status = _clean_booking_status(new_status)
    if new_status not in BOOKING_STATUSES:
        return False, "Status de reserva inválido."

    try:
        booking = _select_booking_for_status_update(conn, int(booking_id))
        if not booking:
            conn.rollback()
            return False, "Reserva não encontrada."

        previous_status = booking["status"]
        if _clean_booking_status(previous_status) == new_status:
            conn.rollback()
            return False, "A reserva já está com este status."
        ...
        if previous_status is None:
            ...
            cur = conn.execute(
                ...
                WHERE id = ?
                  AND status = ?
                """,
                [new_status, booking_id, previous_status],
            )
        if cur.rowcount != 1:
            conn.rollback()
            return False, "A reserva foi alterada por outra sessão. Recarregue e tente novamente."

        _record_booking_status_history(
            conn,
    ...
    *,
    ...
    try:
        ...
        _record_booking_status_history(
            ...
            changed_by_id=changed_by_id if changed_by_id is not None else user_id,
            reason=None,
...
def list_booking_status_history(conn: DatabaseConnection, booking_id: int) -> pd.DataFrame:
    return query_df(
        conn,
        """
        SELECT bsh.id, bsh.booking_id, bsh.previous_status, bsh.new_status,
               bsh.changed_by_id, u.full_name AS changed_by_name,
               bsh.changed_at, bsh.reason, bsh.source, bsh.created_at
    ...
    *,
    ...
    try:
        ...
        cursor = conn.execute(
            ...
            """,
            [reason, inactive_by_id, supply_id],
        )

        if cursor.rowcount != 1:
            conn.rollback()
            return False, "O item foi alterado por outra sessão. Recarregue e tente novamente."
        ...
        conn.rollback()
        return False, f"Erro ao inativar item: {exc}"
...
def reactivate_supply(
    ...
    try:
        ...
        conn.rollback()
        return False, f"Erro ao reativar item: {exc}"
...
def delete_supply_if_unused(
    ...
    try:
        ...
        if blockers:
            ...
            return (
                False,
                "Exclusão definitiva não permitida. "
                "O item possui "
                + ", ".join(blockers)
        ...
        conn.rollback()
        return False, f"Erro ao excluir item: {exc}"
