...
def connect(
    ...
    *,
    ...
    if allow_create:
        ...
        connection_target: str | Path = path
        use_uri = False
    else:
        if not path.is_file():
            raise FileNotFoundError("Banco SQLite ainda não foi inicializado explicitamente.")
        connection_target = f"file:{path.resolve().as_posix()}?mode=rw"
        use_uri = True
    conn = sqlite3.connect(connection_target, check_same_thread=False, uri=use_uri)
